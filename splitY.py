...
scaler.transform(tuning_x_test)
...
# # train 변수 튜닝
tunning_train_x = x_train.apply(tuning_var, axis = 1)
tunning_test_x = x_test.apply(tuning_var, axis = 1)
...
model = model(x_scaled,y_train, epochs=200 )
...
dir(model)
model2=load_model('model_ann_bio_h5') 
model2.evaluate(x_test,y_test)[1]  # 1.61
...
# 진짜 테스트 변수 가공
tunning_realtest_x = test.apply(tunning_train_x, axis = 1)
realtest_x_scaled = m_sacled.transform(tunning_train_x)
...
model_hhb = model(x_scaled, train_y, 200)     #  model_x사용법 : model_x(설명변수, 종속변수, epochs)입니다.
model_hbo2 = model_x(x_scaled, train_y, 200)
model_ca = model_x(x_scaled, train_y, 200)
model_na = model_x(x_scaled, train_y, 200)
